...
class LSegEnc(BaseModel):
    def __init__(
        ...
        **kwargs,
        ...
        self.text = clip.tokenize(self.labels)
        self.token_cached = None
        self.clip_cached = None
        
    def forward(self, x, labelset):
        if isinstance(labelset, torch.Tensor):
            #already tokenized
            text = labelset
        else:
            if labelset == '':
        ...
        if self.token_cached is None or torch.any(torch.asarray([self.token_cached != labelset]) if isinstance(self.token_cached != labelset, bool) else self.token_cached != labelset).item():
            self.token_cached = labelset
            text = text.to(x.device)
            text_features = self.clip_pretrained.encode_text(text)
            self.clip_cached = text_features
        else:
            text_features = self.clip_cached
