...
class OneMap:
    ...
    def fuse_maps_layered(self,
        ...
        if self.fusion_type == FusionType.EMA:
            ...
            self.feature_map[indices[0], indices[1], :, :] = self.feature_map[indices[0], indices[1], :, :] * weight_1.unsqueeze(-1).unsqueeze(-1)
            # + \
            #                                            values_mapped.values().data * weight_2.unsqueeze(-1).unsqueeze(-1)
    ...
    def project_dense_layered(self,
        ...
        # layered grid
        layered_pcl_grid_ids = rotated_pcl[:, :3]
        ...
        obstcl_confidence_mapped_layered = scores_mapped_layered
        ...
        #                                 new_map.element_size() * new_map.values().shape[
        #                                     0] * self.feature_dim / 1024 / 1024))
        return torch.sparse_coo_tensor(all_ids, coalesced_scores, (self.n_cells, self.n_cells, 1), is_coalesced=True).cpu(), torch.sparse_coo_tensor(all_ids_layered, coalesced_map_data_layered, (self.n_cells, self.n_cells, self.n_layers, self.feature_dim), is_coalesced=True).cpu(), obstacle_mapped.cpu(), obstcl_confidence_mapped.cpu()
