...
class OneMap:
    ...
    def update(self,
        ...
        if len(values.shape) == 1 or (values.shape[-1] == 1 and values.shape[-2] == 1):
            ...
            if self.layered:
                ...
                self.fuse_maps_layered(obstacle_mapped, obstcl_confidence_mapped, artifical_obstacles)
            else:
                (confidences_mapped, values_mapped,
    ...
    def project_dense_layered(self,
        ...
        # layered grid
        rotated_pcl_3d = rotated_pcl.clone()
        cam_z = tf_camera_to_episodic[2, 3] / tf_camera_to_episodic[3, 3]
        rotated_pcl_3d[:, :3] += torch.tensor([cam_x, cam_y, cam_z], device='cuda')
        layered_pcl_grid_ids = rotated_pcl_3d[:, :3]
